...
export function ReportsPage({ data }: ReportsPageProps) {
  return (
    <div className="page-stack">
      ...
      <SummaryTable title="按款式汇总" rows={summarizeByStyle(data.sales)} />
      <SummaryTable title="按月份汇总" rows={summarizeByMonth(data.sales)} />
    </div>
  );
}
  ...
  return (
    <section className="panel">
      ...
      <DataTable
        ...
        emptyText="暂无可汇总数据。"
        columns={[
          { header: "分组", render: (row) => row.key },
          { header: "销量", render: (row) => row.quantitySold },
          { header: "收入", render: (row) => `¥${row.totalRevenue.toFixed(2)}` },
          { header: "成本", render: (row) => `¥${row.totalCost.toFixed(2)}` },
          { header: "利润", render: (row) => `¥${row.totalProfit.toFixed(2)}` },
          { header: "利润率", render: (row) => `${(row.averageProfitMargin * 100).toFixed(1)}%` }
        ]}
      />
    </section>
  );
